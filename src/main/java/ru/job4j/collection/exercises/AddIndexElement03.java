...
/**
 * А что если мы хотим добавить элемент не в конец списка,
 * а в какое то конкретное место в списке. Не проблема,
 ...
 *
 * boolean contains(E e) - метод проверяет содержит ли список элемент E,
 ...
 * если этого элемента еще нет в коллекции. При этом метод должен вернуть булево значение. Для этого необходимо сравнить размер дубликата списка и списка в который мы добавляем элемент. Используйте методы contains(E e), add(int index, E e), size().
 */

public class AddIndexElement03 {
    public static boolean addNewElement(List<String> list, int index, String str) {
        ...
        /*} else {*/
            /*return false;*/
        list.add(index, str); /* второй вариант решения задачи при помомщи тернарника */
        return check.contains(str) ? false : (list.size() > check.size());
    }
}